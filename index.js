...
//Start the bot
 ...
 function startUpBot()
 {
    ...
    var intervalID = setInterval(reminder.checkTimes, 500);
 }
        ...
        //If the bot is tagged in the message
			if(msg.mentions.users.get(client.user.id) != null)
			{
					...
					if (incomingMessage === "list reminders") {
						var respond = reminder.listReminders();
						msg.reply(respond);
					}
					else if (incomingMessage.includes("remove reminder ")) {
						var respond = reminder.removeReminder(incomingMessage);
						//msg.reply(respond);
					}
					else if (incomingMessage === "clear reders secret lock hasssssha") {
						var respond = reminder.clearReminders();
						//msg.reply(respond);
					}
						...
						//msg.reply(respond);
						   ...
						   var respond = reminder.remindFriendStart(msg.content, msg.mentions.users, attachment, msg.author.username);
						   //msg.reply(respond);
					}
    		}
